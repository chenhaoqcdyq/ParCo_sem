...
class Text2MotionDataset(Dataset):
    def __init__(self, dataset_name, vq_dir, unit_length, codebook_size, print_warning=False):
        ...
        self.sem_end_idx = codebook_size
        self.sem_pad_idx = codebook_size + 1
        ...
        for name in tqdm(id_list):
            try:
                ...
                sem_token = np.load(pjoin(vq_dir, name + '_sem.npy'))
                m_token_dict['sem'] = sem_token
                    ...
                    for line in lines:
                        try:
                            ...
                            if f_tag == 0.0 and to_tag == 0.0:
                                ...
                                m_token_dict_new = {}
                                sem_token_new = sem_token[int(f_tag*fps/unit_length) : int(to_tag*fps/unit_length)]
                                m_token_dict_new['sem'] = sem_token_new
                                m_token_len = []
                                for p_n in self.parts_name:
    ...
    def __getitem__(self, item):
        ...
        sem_token = m_token_dict['sem']
        sem_token = sem_token.reshape(-1)
        ...
        for m_tokens in [Root, R_Leg, L_Leg, Backbone, R_Arm, L_Arm]:
            ...
            if m_tokens_len+1 < self.max_motion_length:
                ...
                m_tokens_len = self.max_motion_length-1
        ...
        sem_token_len = sem_token.shape[0]
        if sem_token_len + 1 < self.sem_max_length:
            sem_token = np.concatenate([sem_token, np.ones((1), dtype=int) * self.sem_end_idx, np.ones((self.sem_max_length-1-sem_token_len), dtype=int) * self.sem_pad_idx], axis=0)
        else:
            sem_token_len = self.sem_max_length-1
            sem_token = np.concatenate([sem_token[:self.sem_max_length-1], np.ones((1), dtype=int) * self.sem_end_idx], axis=0)
        ...
        return caption, Root, R_Leg, L_Leg, Backbone, R_Arm, L_Arm, token_len, sem_token, sem_token_len
