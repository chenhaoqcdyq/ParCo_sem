...
class VQMotionDatasetBodyPart(data.Dataset):
    def __init__(self, dataset_name, window_size=64, unit_length=4, print_warning=False, strategy='basic', with_clip=False, get_vqvae=False, args=None):
    ...
    def __getitem__(self, item):
        ...
        if self.args.lgvq>0:
            ...
            text_mask = []
        if self.with_clip:
            return motion, text, text_token, text_feature, text_feature_all, text_id, text_mask, motion_mask
        else:
...
def DATALoader(dataset_name,
    ...
    trainSet = VQMotionDatasetBodyPart(dataset_name, window_size=window_size, unit_length=unit_length, args=args)
...
def whole2parts_batch(motion, mode='t2m', window_size=None):
    """
    支持batch维度的whole2parts实现
    Args:
        motion: 形状为(batch_size, nframes, dim)的张量
        mode: 't2m' 或 'kit'
        window_size: 可选的窗口大小
    Returns:
        包含6个部分的列表，每个部分的形状为(batch_size, nframes, part_dim)
    """
    batch_size = motion.shape[0]
    nframes = motion.shape[1]
    ...
    if mode == 't2m':
        ...
        s = 0
        e = 4
        root_data = motion[:, :, s:e]  # [batch_size, nframes, 4]
        s = e
        e = e + (joints_num - 1) * 3
        ric_data = motion[:, :, s:e]  # [batch_size, nframes, (joints_num-1)*3]
        s = e
        e = e + (joints_num - 1) * 6
        rot_data = motion[:, :, s:e]  # [batch_size, nframes, (joints_num-1)*6]
        s = e
        e = e + joints_num * 3
        local_vel = motion[:, :, s:e]  # [batch_size, nframes, joints_num*3]
        s = e
        e = e + 4
        feet = motion[:, :, s:e]  # [batch_size, nframes, 4]

        # 重塑数据
        ric_data = ric_data.reshape(batch_size, nframes, -1, 3)    # [batch_size, nframes, joints_num-1, 3]
        rot_data = rot_data.reshape(batch_size, nframes, -1, 6)    # [batch_size, nframes, joints_num-1, 6]
        local_vel = local_vel.reshape(batch_size, nframes, -1, 3)  # [batch_size, nframes, joints_num, 3]

        # 处理root数据
        root_data = torch.cat([root_data, local_vel[:, :, 0, :]], dim=2)  # [batch_size, nframes, 7]

        # 处理各个部分
        R_L = torch.cat([ric_data[:, :, R_L_idx - 1, :], 
                        rot_data[:, :, R_L_idx - 1, :], 
                        local_vel[:, :, R_L_idx, :]], dim=3)  # [batch_size, nframes, 4, 12]
        L_L = torch.cat([ric_data[:, :, L_L_idx - 1, :], 
                        rot_data[:, :, L_L_idx - 1, :], 
                        local_vel[:, :, L_L_idx, :]], dim=3)  # [batch_size, nframes, 4, 12]
        B = torch.cat([ric_data[:, :, B_idx - 1, :], 
                      rot_data[:, :, B_idx - 1, :], 
                      local_vel[:, :, B_idx, :]], dim=3)  # [batch_size, nframes, 5, 12]
        R_A = torch.cat([ric_data[:, :, R_A_idx - 1, :], 
                        rot_data[:, :, R_A_idx - 1, :], 
                        local_vel[:, :, R_A_idx, :]], dim=3)  # [batch_size, nframes, 5, 12]
        L_A = torch.cat([ric_data[:, :, L_A_idx - 1, :], 
                        rot_data[:, :, L_A_idx - 1, :], 
                        local_vel[:, :, L_A_idx, :]], dim=3)  # [batch_size, nframes, 5, 12]

        # 重塑为最终形状
        Root = root_data  # [batch_size, nframes, 7]
        R_Leg = torch.cat([R_L.reshape(batch_size, nframes, -1), 
                          feet[:, :, 2:]], dim=2)  # [batch_size, nframes, 50]
        L_Leg = torch.cat([L_L.reshape(batch_size, nframes, -1), 
                          feet[:, :, :2]], dim=2)  # [batch_size, nframes, 50]
        ...
        # KIT数据集的处理逻辑类似，这里省略具体实现
        # 实现方式与t2m类似，只是索引和维度不同
        raise NotImplementedError("KIT mode not implemented yet for batch processing")
    else:
        raise ValueError(f"Unsupported mode: {mode}")

    return [Root, R_Leg, L_Leg, Backbone, R_Arm, L_Arm]

def parts2whole_batch(parts, mode='t2m', shared_joint_rec_mode='Avg'):
    """
    支持batch维度的parts2whole实现
    Args:
        parts: 包含6个部分的列表，每个部分的形状为(batch_size, nframes, part_dim)
        mode: 't2m' 或 'kit'
        shared_joint_rec_mode: 共享关节的重建模式，可选'Avg', 'L_Arm', 'R_Arm', 'Backbone'
    Returns:
        形状为(batch_size, nframes, dim)的张量
    """
    assert isinstance(parts, list)
    Root, R_Leg, L_Leg, Backbone, R_Arm, L_Arm = parts
    ...
    if mode == 't2m':
        ...
        # 提取root数据和feet数据
        rec_root_data = Root[..., :4]  # [batch_size, nframes, 4]
        rec_feet = torch.cat([L_Leg[..., -2:], R_Leg[..., -2:]], dim=-1)  # [batch_size, nframes, 4]
        
        # 重塑各个部分
        R_L = R_Leg[..., :-2].reshape(batch_size, nframes, 4, -1)  # [batch_size, nframes, 4, 12]
        L_L = L_Leg[..., :-2].reshape(batch_size, nframes, 4, -1)  # [batch_size, nframes, 4, 12]
        B = Backbone.reshape(batch_size, nframes, 5, -1)  # [batch_size, nframes, 5, 12]
        R_A = R_Arm.reshape(batch_size, nframes, 5, -1)   # [batch_size, nframes, 5, 12]
        L_A = L_Arm.reshape(batch_size, nframes, 5, -1)   # [batch_size, nframes, 5, 12]
        
        # 初始化重建数据
        rec_ric_data = torch.zeros(batch_size, nframes, joints_num-1, 3).to(device, dtype=rec_root_data.dtype)
        rec_rot_data = torch.zeros(batch_size, nframes, joints_num-1, 6).to(device, dtype=rec_root_data.dtype)
        rec_local_vel = torch.zeros(batch_size, nframes, joints_num, 3).to(device, dtype=rec_root_data.dtype)
        rec_local_vel[..., 0, :] = Root[..., 4:]  # 设置root的速度
        ...
        if shared_joint_rec_mode == 'L_Arm':
            ...
            raise ValueError(f"Unsupported shared_joint_rec_mode: {shared_joint_rec_mode}")
        
        # 重塑并连接所有部分
        rec_ric_data = rec_ric_data.reshape(batch_size, nframes, -1)
        rec_rot_data = rec_rot_data.reshape(batch_size, nframes, -1)
        rec_local_vel = rec_local_vel.reshape(batch_size, nframes, -1)
        ...
        # 重塑各个部分
        R_L = R_Leg[..., :-2].reshape(batch_size, nframes, 5, -1)  # [batch_size, nframes, 5, 12]
        L_L = L_Leg[..., :-2].reshape(batch_size, nframes, 5, -1)  # [batch_size, nframes, 5, 12]
        B = Backbone.reshape(batch_size, nframes, 4, -1)  # [batch_size, nframes, 4, 12]
        R_A = R_Arm.reshape(batch_size, nframes, 4, -1)   # [batch_size, nframes, 4, 12]
        L_A = L_Arm.reshape(batch_size, nframes, 4, -1)   # [batch_size, nframes, 4, 12]
